...
from scripts import configs, utils, airtavolo
...
def _get_urlsafe_version(version: str) -> str:
    """Convert version to URL safe format"""
    return version.replace("+", "%2B")

def _get_release_url(version: str) -> str:
    """Get the release URL for the given version"""
    build_version_urlsafe = _get_urlsafe_version(version)
    return f"https://github.com/eirannejad/pyRevit/releases/tag/v{build_version_urlsafe}/"

def _modify_contents(files: List[str], finder: re.Pattern, new_value: str):
    """Modify the contents of files matching the finder pattern with new_value"""
    for text_file in files:
        contents = []
        file_changed = False
        with open(text_file, "r") as sfile:
            logger.debug("Setting version in file %s to %s", text_file, new_value)
...
def _modify_msi_year(copyright_message: str):
    """Modify the MSI file with the new copyright year"""
    msi_file = configs.PYREVIT_COMMON_MSI_PROPSFILE
    namespace = "http://schemas.microsoft.com/developer/msbuild/2003"
    dom = ET.parse(msi_file)
    ET.register_namespace("", namespace)
    nuget = dom.getroot()
    copyright_tag = nuget.findall(f".//{{{namespace}}}Copyright")[0]
...
def _modify_msi_version(_: str, install_version: str):
    """Modify the MSI file with the new version"""
    msi_file = configs.PYREVIT_COMMON_MSI_PROPSFILE
    namespace = "http://schemas.microsoft.com/developer/msbuild/2003"
    dom = ET.parse(msi_file)
    ET.register_namespace("", namespace)
    nuget = dom.getroot()
    version_tag = nuget.findall(f".//{{{namespace}}}Version")[0]
    version_tag.text = install_version
    dom.write(msi_file, encoding="utf-8", xml_declaration=True)

def _modify_choco_nuspec_year(copyright_message: str):
    """Modify the Chocolatey nuspec file with the new copyright year"""
...
def _modify_choco_nuspec_version(build_version: str, install_version: str):
    """Modify the Chocolatey nuspec file with the new version and release notes URL"""
    nuspec_file = configs.PYREVIT_CHOCO_NUSPEC_FILE
    ...
    nuget = dom.getroot()
    version_tag = nuget.findall(f".//{{{namespace}}}version")[0]
    release_notes_tag = nuget.findall(f".//{{{namespace}}}releaseNotes")[0]
    version_tag.text = install_version
    release_notes_tag.text = releasenotes_url
    dom.write(nuspec_file, encoding="utf-8", xml_declaration=True)

def get_version(install=False, url_safe=False) -> str:
    """Get the current version"""
    verfile = configs.PYREVIT_INSTALL_VERSION_FILE if install else configs.PYREVIT_VERSION_FILE
    with open(verfile, "r") as vfile:
        for cline in vfile.readlines():
            if match := VER_FINDER.search(cline):
                return _get_urlsafe_version(match.group()) if url_safe else match.group()
...
def set_year(_: Dict[str, str]):
    ...
    _modify_contents(files=configs.COPYRIGHT_FILES, finder=cp_finder, new_value=new_copyright)
...
def set_ver(args: Dict[str, str]):
    ...
    def _update_build_number(version: str) -> str:
    ...
    if VER_FINDER.match(build_version):
        ...
        _modify_contents(files=configs.VERSION_FILES, finder=VER_FINDER, new_value=build_version)
        ...
        _modify_contents(files=configs.INSTALLER_FILES, finder=VER_FINDER, new_value=install_version)
...
def set_build_ver(args: Dict[str, str]):
    """Generate and set a new build version"""
...
def set_next_ver(args: Dict[str, str]):
    """Increment the version patch number and commit the changes"""

    def _increment_build_number(version: str) -> str:
        parts = VER_PART_FINDER.findall(version)
        if parts:
            version = parts[0]
            major, minor, patch = version[:3]
...
def _find_tbundles(root_path: str) -> List[str]:
    """Find translation bundles in the given root path"""
...
def _extract_title(bundle_dict: dict) -> str:
    """Extract the title from the bundle dictionary"""
...
def _find_tlocale(bundle_title: str, tool_locales: List[ToolLocales]) -> ToolLocales:
    """Find the translation locale that matches the bundle title"""
    for tlocale in tool_locales:
        if bundle_title == tlocale.name:
            return tlocale

def _prepare_title(langs_dict: dict) -> dict:
    """Prepare the title dictionary for locales"""
    tdata = {}
...
def _update_locales(bundle_path: str, tool_locales: List[ToolLocales]):
    """Update the locales in the bundle path"""
    blfinder = re.compile(r"(.*bundle\.yaml)")
    bundlefile_match = next((x for x in os.listdir(bundle_path) if blfinder.match(x)), None)
