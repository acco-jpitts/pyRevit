...
def clean_build(_: Dict[str, str]):
    ...
    for dirname, subdirs, _ in os.walk(configs.DEVPATH):
        for subdir in subdirs:
            if subdir in ARTIFACTS:
                shutil.rmtree(op.join(dirname, subdir))
                print(f"Removed {op.join(dirname, subdir)}")
...
def build_binaries(_: Dict[str, str]):
    ...
    for file in file_list:
        if file.endswith((".dll", ".png", ".json")):
            src_file = op.join(configs.BINPATH_NETCORE, file)
            dest_file = op.join(configs.ENGINES2PATH_NETCORE, file)
